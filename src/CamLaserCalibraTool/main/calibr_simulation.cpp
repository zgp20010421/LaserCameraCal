...
void GenerateSimData( std::vector<Oberserve>& obs)
{
  ...
  Eigen::Matrix3d Rlc;  // Pl = Rlc*Pc+Tlc
  Rlc << 0 , 0 , 1,
        -1 , 0 , 0,
        0 , -1 , 0;
  ...
  // 随机生成 AprilTag 在相机系下的位姿（Rca, tca）
  std::random_device rd; // 创建一个非确定性随机数生成器
  std::default_random_engine generator(rd()); // 创建一个高性能伪随机数生成引擎
  // random CamPose distribution 生成随机相机的位置 欧拉角 xyz
...
///////////////////////////////////////////////////////////////////////////////////////////////
    ...
    Eigen::Quaterniond qla(Rlc * Rca); // Rla->
    ...
    {
      double theta = -M_PI_2 + j * M_PI/180; // [-90， 90]
