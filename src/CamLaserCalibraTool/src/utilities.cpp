...
void TranScanToPoints(const sensor_msgs::LaserScan& scan_in,std::vector<Eigen::Vector3d>& Points)
{
    ...
    // printf("TranScanToPoints n_pts: %ld\n", n_pts);
    // printf("scan_in.angle_increment: %lf\n", scan_in.angle_increment);
    ...
    {
        double angle_deg = RAD2DEG(scan_in.angle_min + i * scan_in.angle_increment);
        // printf("angle_deg: %f\n", angle_deg);
        if(std::fabs(angle_deg) > 150){ // 修改采集激光数据
    ...
    for (size_t i = 0; i < n_pts; ++i) {
        ...
        {
            Points.push_back(Eigen::Vector3d(0.0,0.0, 0) );
        }
